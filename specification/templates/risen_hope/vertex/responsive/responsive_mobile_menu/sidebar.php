...
foreach ($list as $i => &$item) :
	...
	if ($item->level==1) {
		if ($item->deeper) {
		echo "<h3 onclick='s5_responsive_mobile_sidebar_h3_click(this.id)'><span class='s5_sidebar_deeper'><a ".$class." href='".$item->flink."' >".$item->title."</a></span></h3>";
		
		echo "<div class='s5_responsive_mobile_sidebar_sub'>";
		}
		else {
		echo "<h3><span><a ".$class."  href='".$item->flink."' >".$item->title."</a></span></h3>";
		continue;
		echo "<div class='s5_responsive_mobile_sidebar_sub'>";
